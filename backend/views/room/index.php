	...
	<div class="searchResult">
		...
		<table id="room_table">
			...
			<tbody>
				...
				<tr>
					<td><input type="checkbox" name="ids[]" value="<?php echo $row['room_id']?>"></input></td>
					<td><?php echo ($key+1)?></td>
					<td><?php echo $row['room_name']?></td>
					<td><?php echo $row['status']==1?"啓用":"禁用";?></td>
					<td class="op_btn">
						<a href="<?php echo Url::toRoute(['room/edit','id'=>$row['room_id']]);?>"><img src="<?php echo $baseUrl; ?>images/write.png"></a>
						<a id="<?php echo $row['room_id']?>" class="delete"><img src="<?php echo $baseUrl; ?>images/delete.png"></a>
					</td>
				</tr>
				<?php }?>
			</tbody>
		...
		<div class="btn">
			<a href="<?php echo Url::toRoute(['room/add']);?>"><input type="button" value="添加"></input></a>
	...
	?>
		$('#page_div').jqPaginator({
		    ...
		    onPageChange: function (num, type) {
		    	...
		    	$.ajax({
	                url:"<?php echo Url::toRoute(['getroompage']);?>",
	                type:'get',
	                data:'pag='+num,
	             	dataType:'json',
	            	success:function(data){
	            		...
	            		if(data != 0){
	    	                $.each(data,function(key){
								...
								str += '<td>'+(key+1)+'</td>';
								str += '<td>'+data[key]['room_name']+'</td>';
								var state = data[key]['status']==1?"启用":"禁用";
								str += '<td>'+state+'</td>';
								str += '<td class="op_btn">';
								str += '<a href="<?php echo Url::toRoute(['room/edit']);?>?id='+data[key]['room_id']+'"><img src="<?php echo $baseUrl; ?>images/write.png" ></a>';
								str += '<a class="delete" id="'+data[key]['room_id']+'" ><img src="<?php echo $baseUrl; ?>images/delete.png"></a>';
								str += '</td>';
								str += '</tr>';

	                          });
	    	                $("table#room_table > tbody").html(str);
	    	            }
	            	}
	            });
